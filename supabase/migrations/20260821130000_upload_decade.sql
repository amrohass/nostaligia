...
-- to the 1960s. claim_upload_slot reads its keys out of the draft and has never read a date.
-- So the answer went into the request and nowhere else, and every member upload arrived with
-- date_earliest null, date_precision null and the generated `decade` column null.
--
...
-- decade visible in three places at once and therefore the one in which its absence stopped
-- looking like an empty column and started looking like a lost answer.
--
...
--
-- Absent is still valid: a draft with no decade inserts nulls, exactly as before.
-- posts_date_precision_needs_a_date allows both columns null, the archive has items whose
-- decade genuinely is not known, and refusing an upload over it would be inventing a
-- requirement §3 does not make.
--
-- ── THE BODY BELOW IS 0032's, NOT 0029's ─────────────────────
--
-- claim_upload_slot has been defined three times: 0029 created it, 0032 added §7's rights
-- capture — licence allowlist, provenance, stamped consent — and this file adds the date.
-- CREATE OR REPLACE takes whatever text it is given, so rebuilding from 0029 silently
-- reverts 0032 and every member upload goes back to arriving with no licence, which
-- posts_approved_has_rights then refuses to let anyone approve. That is exactly the failure
-- 0036's header records against posts_write_audit, and the first draft of THIS file made
-- it: 11_claim_upload_slot went from 30 passing to 13 failing and named every rights
-- assertion. The suite caught it; the grep that should have preceded it did not happen.
--
-- So: the body is copied from 0032 and altered in exactly three places — two new declared
-- variables, one validation block, and three columns in the INSERT.
...
comment on function public.claim_upload_slot(bigint, text, public.post_kind, jsonb) is
  'Charges the daily quota and creates the draft post in one transaction, with §7 rights capture as a precondition and §3''s decade expanded into an EDTF-lite range (CLAUDE.md §2, §3, §6, §7).';
