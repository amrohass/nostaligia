...
3. One restore tested from a backup held by the maintainer. *Built 1 Sep 2026 —
   `scripts/backup.ts` and `scripts/restore-verify.ts`. Not yet discharged: it needs a second
   Cloudflare account and a scratch Supabase project, both provisioning rather than code. See
   below.*
...
## Backups (gate 3)
    ...
    deno run --allow-run --allow-net --allow-env --allow-read --allow-write \
      scripts/backup.ts --dry-run          # everything except the destination writes
    deno run ... scripts/backup.ts                    # a real run
    deno run ... scripts/backup.ts --pin pre-launch   # a copy nothing prunes
    deno run --allow-read scripts/backup.ts --selftest

Three decisions, made 31 Aug 2026: the self-held copy is **a second R2 bucket under a
different Cloudflare account**; the cadence is **weekly full database + incremental originals**
plus snapshots pinned forever at pre-launch and immediately after the seed import; the restore
target is **a scratch Supabase project**.

"A different account" is checked rather than trusted — a destination whose account id equals
the source's is refused by name, because a backup inside the blast radius of the thing it
backs up is not a backup, and a typo in an env file is exactly how that happens.

### `supabase db dump` does not emit triggers

Measured 1 Sep 2026 against the deployed database, and it is the finding that decides whether
this backup is worth anything:
...
The trigger *functions* are all there. Only the bindings are missing — and nearly every
...
mandatory handle), and every `*_bump_content_*` the publisher depends on. A restore without
them has every row, every policy, every grant and every function; it errors nowhere; and its
audit log is editable, its comments unfiltered, and nothing it does ever publishes.

So there is a **fifth dump**, `triggers.sql`, reconstructed from `pg_get_triggerdef()` — from
the catalogue rather than a hand-written list, so it cannot drift when a trigger is added. And
the counting that found it is now part of the job: every run compares the catalogue against
the dump text and **a real run refuses to store an incomplete one**. Verified by mutation —
remove the fifth dump and it reports `46 in the database, 0 in the dump`.

### What is and is not copied

`originals/` always, incrementally, and the object list comes from `media_assets` rather than
from a bucket listing — so the backup covers exactly what the restored database will reference,
and an object the database names but the bucket does not have is reported as an archive
problem. The basemap is copied once, not weekly: it is 174 MB of the 176 MB `public` bucket and
changes about yearly. `quarantine/` never — it is unvalidated uploads. The vault secret and the
`pg_cron` schedule are deliberately not carried into the second account and are re-set by hand
on a restored project.

The dumps are AES-256-GCM under `BACKUP_PASSPHRASE`, and a real run **decrypts what it just
uploaded** before reporting success, so a passphrase that does not work is discovered on the
day of the backup rather than the day of the restore. The originals are *not* encrypted by
this script and the header says why: Web Crypto has no streaming AEAD, and a chunked framing
would be a file format invented here to hold the archive's only irreplaceable bytes.

### Still needed, and neither is code

`supabase/functions/.backup.vars` (git-ignored, and the pre-commit hook blocks a force-add)
with `BACKUP_R2_ACCOUNT_ID`, `BACKUP_R2_ACCESS_KEY_ID`, `BACKUP_R2_SECRET_ACCESS_KEY`,
`BACKUP_R2_BUCKET`, `BACKUP_PASSPHRASE` — and a scratch Supabase project to restore into.
`scripts/restore-verify.ts` holds the checks and refuses the production ref by name; its
strongest check is not a query but running the whole pgTAP suite against the restored
database, `05_matrix` included, which *is* gate 1.
